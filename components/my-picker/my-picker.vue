<template>
    <u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" :show="value" length="auto"
        ...
        <view class="u-datetime-picker">
            ...
            <view class="u-picker-body">
                <picker-view v-if="mode == 'selector'" :value="valueArr" @change="change" class="u-picker-view"
                    @pickstart="pickstart" @pickend="pickend" @touchend="changeHandlerHide">
                    <picker-view-column v-if="!reset">
                        <view class="u-column-item" v-for="(item, index) in range" :key="index"
                            @click.stop="pickerColumnHandler(index)">
...
<style lang="scss" scoped>
	...
	::v-deep .uni-picker-view-content {
		padding: 0 0 calc(1000rpx - 34px) 0 !important;
		// transform: translateY(0) translateZ(0px) !important;
	}

	::v-deep .uni-picker-view-mask {
		background: linear-gradient(0deg, hsla(0, 0%, 100%, 0.95), hsla(0, 0%, 100%, .6));
		background-position: center bottom;
		background-size: 100% calc(100% - 44px) !important;
