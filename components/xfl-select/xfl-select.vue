<template>
    <div class="show-box" :class="{disabled: disabled, active: isShowList}" :style="style_Container">
        ...
        <input v-if="showInput" class="input" placeholder-style="color: #bbb;" type="text" v-model="selectText"
            :placeholder="placeholder" @focus="onFocus" @blur="onBlur" @input="onInput"
        ...
        <!-- 右侧的小三角图标 -->
        <span @click="onUpperClick" class="iconfont iconarrowBottom-fill right-arrow"> <u-icon slot="right"
        ...
        <div class="list-container" @click.stop="onListClick" :style="'top:' + listTop__ + 'px;'" v-show="isShowList">
            ...
            <scroll-view class="list" style="background-color: #fff;" :style="'max-height: ' + listBoxHeight__ +'em;'"
                ...
                <div class="item" @click="onClickItem(index, item.value,item)" v-for="(item, index) in innerList"
                    :key="index" v-show="showItem(item.value)"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            innerList() { // 转换列表的数据格式
                ...
                orginArr.forEach((val, index) => {
                    ...
                    let id = typeof val === 'object' && 'id' in val ? val.id : val;
                    arr.push({
                        isActive: false,
                        value: value,
                        id: id,
        ...
        methods: {
            showItem(val) {
                return val.indexOf(this.inputVal) != -1
            },
            ...
            onClickItem(index, value, item) { // 列表项上的点击事件
                if (this.itemIsDisabled(index)) {
    ...
    function getNodeInfo({
        ...
        stepRunFunc((next, currNum) => {
            selectorQuery.exec(([data]) => { // 开始查询页面中的节点
                ...
                data || totalAttemptNum <= currNum ? typeof callback === 'function' && callback.call(
                    thisObj, data) : next(attemptSpaceTime);
